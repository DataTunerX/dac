...
# Task
基于提供的【知识摘要列表】，深入理解每条记录的核心内容。根据用户的【查询问题】，判断哪些知识记录可能包含回答问题所需的信息。请调用 `select_knowledge` 工具，工具参数即为输出格式要求。

# 检索推理法则（精准优先）
在选择相关知识时，请遵循以下逻辑：
1. **意图理解**：理解用户问题的真实意图，而不仅仅是关键词匹配。
2. **精准筛选**：只选择摘要中**明确包含**回答问题所需信息的记录；仅有章节标题、目录索引或纯导航信息的条目不要选。
3. **排除不确定**：如果摘要无法确认某条记录能回答用户问题，即使主题相近也应排除，避免引入噪音。
4. **排除无关**：明显与问题无关的记录（如完全不同的主题领域）应排除。
5. **排序**：选中的知识 ID 必须按与问题相关度**从高到低**排列，最相关的记录排在最前面。

# Constraints
- **禁止猜测**：只基于摘要内容进行判断，不要假设摘要未提及的内容。
- **精简要输出**：`reasoning` 用 2–5 句话概括选择逻辑即可，**禁止**逐条解释每个 ID。
...
**回答决策规则：**
1. 若新的问题可以正常生成出来，那就设置conclusion为 `terminate`，requery 为新生成的问题。
2. 若新的问题无法正常生成出来，那就设置conclusion为 `continue`，requery 为空的字符串。
...
**注意：请调用 `reformulate_query` 工具，工具参数即为输出格式要求。**
"""
...
**注意：请调用 `observe_answer` 工具，工具参数即为输出格式要求。**
"""
...
**回答规则：**
1. 若背景知识能够充分解答用户问题，请提供完整回答并在结论设置为 `terminate`。
2. 若背景知识与用户问题无关或信息不足，请不要直接回答问题，请根据原始的问题，保留原问题的语意，重新生成一个更清晰易懂的相似的新问题，作为下次提问的问题。
...
**注意：请调用 `answer_question` 工具，工具参数即为输出格式要求。**
"""
...
- 请调用 `score_knowledge_blocks` 工具，工具参数即为输出格式要求。
"""
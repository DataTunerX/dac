...
from .doc_agent import DocAgentExecutor
...
def main(host, port, agent_card, redis_host, redis_port, redis_db, password, provider, api_key, base_url, model, temperature, heartbeat_interval, stream, max_steps):
    ...
    try:
        ...
        descriptor_types_json_string = os.getenv('DescriptorTypes', '')
        logger.info(f"DescriptorTypes is: {descriptor_types_json_string[:200] if descriptor_types_json_string else 'None'}...")
        ...
        request_handler = DefaultRequestHandler(
            agent_executor=DocAgentExecutor(
                provider=provider,
                api_key=api_key,
                base_url=base_url,
                model=model,
                ...
                descriptor_types_json_string=descriptor_types_json_string,

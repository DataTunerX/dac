...
class OrchestratorAgent(BaseAgent):
    ...
    async def get_plan(self, query) -> TaskList:
        ...
        if len(self.agent_cards) == 1:
            ...
            return TaskList(
                thought_process=f"Only one agent available ({sole_agent.name}), direct dispatch without LLM planning.",
        ...
        steps = await self.planner_agent.make_plan(query, self.agent_cards)
        if steps is None:
            return None
            ...
            for task in current_tasks.tasks:
                ...
                if stream:
                    try:
                        ...
                        elif self.metadata.get('answer_model') == 'original':
                            # answer_model=original 模式下，expert/code agent 跳过了 observe_common 验证，
                            # 返回的原始内容不含 "reason:The current answer addresses the question very well." 标记，
                            # 因此直接视为 complete，不走 get_last_step_status 检查
                            current_task_status = "complete"
                            logger.info(f">>>>>> [answer_model=original] OrchestratorAgent.a2a_tasks() Task {task.id} 跳过 get_last_step_status 检查，直接标记为 complete <<<<<<")
                        else:
                            # # 用完整拼接结果判断成功/失败，避免流式返回时最后一 chunk 不含 reason 导致误判为 fail
    ...
    async def add_history(self, query, final_answer, knowledge):
        final_answer_str = "".join(final_answer)
        logger.info(f"add_history metadata : user_id: {self.metadata['user_id']}, agent_id:{self.metadata['agent_id']}, run_id:{self.metadata['run_id']}")
    ...
    async def stream(self, query, task_knowledges, think) -> AsyncIterable[dict[str, Any]]:
        ...
        # add memory, ds no need to controller memory

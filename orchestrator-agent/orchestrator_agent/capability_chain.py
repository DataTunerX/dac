...
class InputItem(BaseModel):
    """A single input item needed by a step, with its source classification."""
...
class StepEvaluation(BaseModel):
    """Per-step capability evaluation with I/D/O/R/C dimensions."""
...
class AggregatedCapability:
    """Aggregated capability result from chain-scoring arithmetic.

    Attributes:
        can_handle: Whether this agent can independently handle the full query.
        can_contribute: Whether this agent can contribute to at least one step.
        confidence: 0.0–1.0 confidence derived from step scores.
        handle_score: Arithmetic mean of all step scores.
        threshold: The threshold used for the decision.
        contributing_steps: IDs of steps whose score >= threshold.
        step_scores: Per-step score mapping.
        has_external_dependency: Whether any step depends on external input.
        contribution: The contribution description text.
        missing_requirements: Requirements this agent cannot provide itself.
    """
...
def aggregate(result: CapabilityChainResult, threshold: float | None = None) -> AggregatedCapability:
    ...
    Args:
        result: The parsed CapabilityChainResult from the LLM.
        threshold: Optional override for the env-based threshold.

    Returns:
        AggregatedCapability with can_handle, can_contribute, and confidence computed.
    """
    ...
    # specifies output flow (is_final=true for final results, is_final=false for
    # intermediate).
    ...
    # can_contribute has two paths:
    # 1. can_handle       — can handle independently, naturally can also contribute
    # 2. contributing non-empty — at least one step's score reaches the threshold
    #
    # We no longer gate on contribution text being non-empty, because the LLM
    # frequently forgets to output it. The contribution text is preserved as
...
def parse_chain_result(data: dict[str, Any]) -> CapabilityChainResult:
    ...
    """
    # Filter out fields the LLM may inject but the schema does not need

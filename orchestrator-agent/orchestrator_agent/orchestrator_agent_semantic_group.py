...
- **严格禁止曲解问题**：不得因为本 SG 只覆盖某类业务而把问题的业务领域替换成自己能处理的领域。
  例如：问题="公司辞退员工的补偿标准（劳动法）"，SG 覆盖"商品订单"时，步骤不能写成"查询退款政策"——这是曲解。
  又例：问题="工伤认定流程"，SG 覆盖"请假审批"时，步骤不能写成"查询请假流程"——这是曲解。
  步骤描述必须忠实于问题原文的业务领域，不能偷换概念。
...
- risks：不影响分值的风险提示。
- reason：前置检查结论（"领域交集：[明确有/明确无/不确定] — 依据…"）放在第一句；然后逐步骤一行；最后一句给整体结论。
...
{{
  ...
  "risks": [],
  "reason": "领域交集：明确有 — SG 描述覆盖订单域，问题领域为订单查询。步骤1（用户名→订单列表）：I=1/1 D=2/2 O=1.0 R=1/1 C=1/1，SG覆盖订单域含订单表所有字段。可独立完成。"
}}"""

...
type TenantQuery struct {
	...
	withNamespaces *TenantNamespaceQuery
	withRoles      *TenantRoleQuery
	withMembers    *TenantUserQuery
...
func (_q *TenantQuery) QueryNamespaces() *TenantNamespaceQuery {
	query := (&TenantNamespaceClient{config: _q.config}).Query()
...
func (_q *TenantQuery) QueryMembers() *TenantUserQuery {
...
func (_q *TenantQuery) WithNamespaces(opts ...func(*TenantNamespaceQuery)) *TenantQuery {
	query := (&TenantNamespaceClient{config: _q.config}).Query()
...
func (_q *TenantQuery) WithMembers(opts ...func(*TenantUserQuery)) *TenantQuery {
	query := (&TenantUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withNamespaces; query != nil {
		if err := _q.loadNamespaces(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Namespaces = []*TenantNamespace{} },
			func(n *Tenant, e *TenantNamespace) { n.Edges.Namespaces = append(n.Edges.Namespaces, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withMembers; query != nil {
		if err := _q.loadMembers(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Members = []*TenantUser{} },
			func(n *Tenant, e *TenantUser) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TenantQuery) loadNamespaces(ctx context.Context, query *TenantNamespaceQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantNamespace)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantnamespace.FieldTenantID)
	}
	query.Where(predicate.TenantNamespace(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.NamespacesColumn), fks...))
	}))
...
func (_q *TenantQuery) loadMembers(ctx context.Context, query *TenantUserQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)

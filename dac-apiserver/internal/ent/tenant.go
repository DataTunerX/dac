...
type TenantEdges struct {
	...
	Members []*TenantUser `json:"members,omitempty"`
...
func (e TenantEdges) NamespacesOrErr() ([]*TenantNamespace, error) {
	if e.loadedTypes[0] {
...
// was not loaded in eager-loading.
func (e TenantEdges) MembersOrErr() ([]*TenantUser, error) {
	if e.loadedTypes[2] {
...
func (*Tenant) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case tenant.FieldCode, tenant.FieldName, tenant.FieldStatus, tenant.FieldDescription:
...
// QueryNamespaces queries the "namespaces" edge of the Tenant entity.
func (_m *Tenant) QueryNamespaces() *TenantNamespaceQuery {
	return NewTenantClient(_m.config).QueryNamespaces(_m)
}
...
func (_m *Tenant) QueryMembers() *TenantUserQuery {

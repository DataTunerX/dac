...
func (_u *UserUpdate) AddRunIDs(ids ...uuid.UUID) *UserUpdate {
	_u.mutation.AddRunIDs(ids...)
	return _u
}

// AddRuns adds the "runs" edges to the Run entity.
func (_u *UserUpdate) AddRuns(v ...*Run) *UserUpdate {
...
func (_u *UserUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.RunsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.RunsTable,
			Columns: []string{user.RunsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(run.FieldID, field.TypeUUID),
			},
		}
...
// AddRunIDs adds the "runs" edge to the Run entity by IDs.
func (_u *UserUpdateOne) AddRunIDs(ids ...uuid.UUID) *UserUpdateOne {
	_u.mutation.AddRunIDs(ids...)
	return _u
}

// AddRuns adds the "runs" edges to the Run entity.
func (_u *UserUpdateOne) AddRuns(v ...*Run) *UserUpdateOne {
	ids := make([]uuid.UUID, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
// ClearRuns clears all "runs" edges to the Run entity.
func (_u *UserUpdateOne) ClearRuns() *UserUpdateOne {
	_u.mutation.ClearRuns()
	return _u
}

// RemoveRunIDs removes the "runs" edge to Run entities by IDs.
func (_u *UserUpdateOne) RemoveRunIDs(ids ...uuid.UUID) *UserUpdateOne {
	_u.mutation.RemoveRunIDs(ids...)
	return _u
}

// RemoveRuns removes "runs" edges to Run entities.
func (_u *UserUpdateOne) RemoveRuns(v ...*Run) *UserUpdateOne {
	ids := make([]uuid.UUID, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}

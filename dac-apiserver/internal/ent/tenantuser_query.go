...
func (_q *TenantUserQuery) QueryRole() *TenantRoleQuery {
...
func (_q *TenantUserQuery) WithRole(opts ...func(*TenantRoleQuery)) *TenantUserQuery {
	query := (&TenantRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantUserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TenantUser, error) {
	...
	if query := _q.withRole; query != nil {
		if err := _q.loadRole(ctx, query, nodes, nil,
			func(n *TenantUser, e *TenantRole) { n.Edges.Role = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TenantUserQuery) loadRole(ctx context.Context, query *TenantRoleQuery, nodes []*TenantUser, init func(*TenantUser), assign func(*TenantUser, *TenantRole)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*TenantUser)
	for i := range nodes {
		fk := nodes[i].RoleID
...
func (_q *TenantUserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withRole != nil {
			_spec.Node.AddColumnOnce(tenantuser.FieldRoleID)
		}
	}

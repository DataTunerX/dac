...
2. **成功时**：`status == "success"` 只是说明「某一步操作」成功了，**不等于整个任务完成了**。你需要根据 skill 说明判断当前处于任务流的哪个阶段：
   - 对照 skill 说明中列出的步骤/子任务清单，确认是否每个步骤都已执行并获取了结果。
   - 若 skill 说明中未明确列出步骤，则自问：当前获得的信息是否已经足以**完整回答用户问题**？如果答案还需要进一步加工、转换、归纳或验证，就说明任务尚未完成。
   - **只有当 skill 定义的所有子任务 / 步骤都已成功执行完毕，并且收集到的信息足以形成完整答案时，才能调用 `finish`。**
3. **调用 `finish` 前的强制自检清单**（请在思考中逐项确认，全部满足才能调用 `finish`）：
   - [ ] skill 说明中列出的所有步骤/子任务是否都已执行完毕？
   - [ ] 是否还有未执行的脚本、未读取的文件、未验证的假设？
   - [ ] 最终答案中的每一个事实是否都有工具返回的原始数据支撑（而非推测）？
   - [ ] 当前这一步只是获取了原始数据，还是已经完成了对数据的加工、分析和归纳？
   - [ ] 如果以上任一答案为「否」，**禁止调用 `finish`**，必须继续执行。
4. **失败时**：`status == "error"` 或 `status == "blocked"` 时：
   - 先执行下面「从失败中学习」的步骤，再决定下一步。
   - 根据错误的性质决定是重试、换思路、还是放弃。
5. **信息不足时**：不要猜测，用工具继续探测。
6. **禁止**：
...
def finish_tool(final_answer: str) -> str:
    ...
    调用前必须确认以下所有条件均已满足，否则不得调用：
    1. skill 说明中定义的所有步骤/子任务均已执行完毕
    2. 最终答案中每个事实都有工具返回的原始数据支撑
    3. 不存在未执行的脚本、未读取的文件、未验证的假设
    4. 当前获得的数据已完成加工、分析和归纳，而非仅获取了原始数据
    如果任一条件不满足，请先继续执行缺失的步骤，而非调用 finish。
    """
    return final_answer

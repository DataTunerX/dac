"""
Wikibooks API 数据抓取模块（中文维基教科书等）
页面正文通过 wikipediaapi 访问 MediaWiki；API 主机为 *.wikibooks.org。
...
class WikibookScraper(MediaWikiScraper):
    """维基教科书数据抓取器"""
    ...
    def __init__(self, language: str = "zh", user_agent: str = None):
        super().__init__(
            language,
            user_agent,
            api_domain="wikibooks.org",
            default_user_agent="WikibookScraper/1.0 (Educational Project; contact@example.com)",
        )


class WikipediaAdvancedScraper(WikibookScraper, MediaWikiAdvancedScraper):
    """高级维基教科书抓取器（沿用历史类名 WikipediaAdvancedScraper）。"""

---
name: code_execution
description: 需要由模型在受限 Python 沙箱中写代码、执行并自检来完成：数值/统计/聚合/清洗/小型数据处理。执行时**必须**调用 Runner 提供的 `code_exec` 工具（不要依赖本包内 shell 脚本作为主路径）。重点要注意的是：这里不负责处理根据数据来生成各种图的功能，比如柱状图，饼状图，折线图，条形图，散点图，雷达图，环形图，漏斗图，箱线图，甘特图，流程图，er图，热力图，时序图，直方图等等。
---

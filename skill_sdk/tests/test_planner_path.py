"""Test the original Planner path (use_skill_search=False) — all skills passed directly to LLM.

This tests the full-skills path where the LLM receives all skill descriptions
in a single prompt and selects via tool-calling (select_skill / no_suitable_skill).

Run with:
    cd /Users/james/daocloud/code/dac/skill_sdk
    python tests/test_planner_path.py
"""
...
# ---------------------------------------------------------------------------
# Test case definition (same cases as skill_search advanced test)
# ---------------------------------------------------------------------------
...
TEST_CASES = [
    ...
    TestCase(
        ...
        expected_skill="wordcount",
        explanation="核心需求是统计行数，其他都是附带干扰",
    ),
    TestCase(
    ...
    TestCase(
        ...
        explanation="目录级 hash 校验是 bundle-hash（不是 hashgen）",
    ),
    TestCase(
    ...
    TestCase(
        ...
        expected_skill="pwdgen",
        explanation="随机 token 生成本质是密码生成（不是 uuidgen）",
    ),
    TestCase(
    ...
    TestCase(
        ...
        expected_skill="github",
        explanation="查询 GitHub release 信息",
    ),
    ...
    TestCase(
        ...
        expected_skill=None,
        explanation="code_execution 明确声明不负责画图",
    ),
    TestCase(
...
async def run_single_test(
    ...
    planner_step = await runner.make_plan(
        query=tc.query,
        user_id="test_user",
        run_id=run_id,
        trace_id=trace_id,
    ...
    if planner_step.declined:
        ...
        actual = planner_step.skill if planner_step.skill else None
    ...
    return TestResult(
        ...
        declined=planner_step.declined,
...
async def main():
    print_section("Planner 路径测试 — use_skill_search=False（全量 skills 直接给 LLM）")
    ...
    runner = SkillRunner(
        ...
        max_steps=20,
        cmd_timeout_sec=60,
        use_skill_search=False,  # <-- Planner path: all skills → LLM directly
    )

    skill_count = await load_skills(runner, exclude={"code_execution"})
    print(f"  Loaded {skill_count} skills\n")
    print(f"  All {skill_count} skills passed directly to the Planner LLM prompt.\n")
    ...
    # Print loaded skill names for reference
    skill_names = [s.name for s in runner.lister.skills]
    print(f"  Skill names: {', '.join(sorted(skill_names))}\n")

    results: list[TestResult] = []
    total_tests = 0
    run_id_base = uuid.uuid4().hex[:12]
    ...
    for tc in active_cases:
        ...
        print(f"{status} ({result.elapsed_ms:.0f}ms){extra}")
    ...
    # Detail table
    print(f"\n  {'ID':>3s}  {'类别':10s}  {'预期':20s}  {'实际':20s}  {'结果':6s}  {'耗时':>6s}  {'说明'}")
    print(f"  {'-'*3}  {'-'*10}  {'-'*20}  {'-'*20}  {'-'*6}  {'-'*6}  {'-'*55}")
    for r in results:
        ...
        print(f"  {r.case_id:3d}  {r.category:10s}  {exp:20s}  {act:20s}  {status:6s}  {r.elapsed_ms:5.0f}ms  {detail}")

...
@dataclass
class CodebaseIndexerItem:
    """Codebase indexer item from data services."""
    codebase_indexer_id: str
...
@dataclass
class CodebaseIndexerResponse:
    """Response for single codebase indexer operations."""
    status: str
    data: Optional[CodebaseIndexerItem] = None
    message: Optional[str] = None


@dataclass
class CodebaseIndexerListResponse:
    """Response for codebase indexer list operations."""
    status: str
    data: List[CodebaseIndexerItem]
    count: int = 0


@dataclass
class CodebaseIndexerExistsResponse:
    """Response for codebase indexer exists check."""
    status: str
    exists: bool


@dataclass
class CodebaseIndexerCountResponse:
    """Response for codebase indexer count."""
    status: str
    total_count: int
...
class MetadataValuesResult:
    ...
    def extract_metadata_as_string(self, text_key: str = "metadata_value", id_key: str = "id") -> str:
        """
        Extract id and text from all metadata values and concatenate them.
        Each text block is separated by newline and =========.

        Args:
            text_key: The key to extract text from each metadata item (default: "metadata_value").
            id_key: The key to extract id from each metadata item (default: "id").

        Returns:
            str: Concatenated text content with id and separators.
        """
        texts = []
    ...
    def extract_metadata_as_batches(self, max_chars_per_batch: int = 60000,
                                      ...
                                      id_key: str = "id") -> list:
        """
        将所有文件摘要按字符数上限分批，切分点对齐到完整记录边界。
        
        逐条累加字符数，当加入下一条记录会超过 max_chars_per_batch 时，
        在当前位置切分（取整到记录边界），保证每条记录完整不被截断。
        
        例如：限制 60000 字符，前 100 条共 58000 字符，第 101 条加入后变成 60001，
        则前 100 条作为一个批次（58000 字符），第 101 条开始新批次。
...
class DataServicesClient:
                ...
                if response.status == 200:
                    try:
                        ...
                        return MetadataValuesResult(
                            status=data.get("status", ""),
                            data=data.get("results", {}),  # API returns "results" not "data"
    ...
    # ==================== Codebase Indexer APIs ====================

    async def get_codebase_indexer_by_id(self, codebase_indexer_id: str) -> Optional[CodebaseIndexerResponse]:
        """
        Get codebase indexer by ID.

        Args:
            codebase_indexer_id: Codebase indexer ID.

        Returns:
            CodebaseIndexerResponse with data, or None if not found or request failed.
        """
        url = f"{self.base_url}/codebase_indexers/{codebase_indexer_id}"
        try:
            await self._create_session()
            async with self.session.get(url) as response:
                if response.status != 200:
                    logger.warning(f"get_codebase_indexer_by_id HTTP {response.status}: {await response.text()}")
                    return None
                data = await response.json()
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.error(f"get_codebase_indexer_by_id request error: {e}")
            return None
        ...
        item = CodebaseIndexerItem(
            codebase_indexer_id=item_data.get("codebase_indexer_id", ""),
            filepath=item_data.get("filepath"),
            code_deep_analysis=item_data.get("code_deep_analysis"),
            dd_namespace=item_data.get("dd_namespace"),
            dd_name=item_data.get("dd_name"),
    ...
    async def search_codebase_indexers_by_dd(
        self,
        dd_namespace: str,
        dd_name: str
    ) -> Optional[CodebaseIndexerListResponse]:
        """
        Search codebase indexers by DD namespace and name.
        ...
        Returns:
            CodebaseIndexerListResponse with list of items, or None if request failed.
        """
        url = f"{self.base_url}/codebase_indexers/search/by-dd"
        payload = {
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status != 200:
                    logger.warning(f"search_codebase_indexers_by_dd HTTP {response.status}: {await response.text()}")
                    return None
                data = await response.json()
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.error(f"search_codebase_indexers_by_dd request error: {e}")
            return None
        ...
        for item_data in items_data:
            items.append(CodebaseIndexerItem(
                codebase_indexer_id=item_data.get("codebase_indexer_id", ""),
                filepath=item_data.get("filepath"),
                code_deep_analysis=item_data.get("code_deep_analysis"),
                dd_namespace=item_data.get("dd_namespace"),
    ...
    async def search_codebase_indexers_by_filepath(
        self,
        filepath: str,
        ...
        """
        Search codebase indexers by filepath.

        Args:
            filepath: File path to search for.
            dd_namespace: Optional DD namespace filter.
            dd_name: Optional DD name filter.
            prefix_match: If True, use prefix matching (LIKE query).

        Returns:
            CodebaseIndexerListResponse with list of items, or None if request failed.
        """
        url = f"{self.base_url}/codebase_indexers/search/by-filepath"
        payload = {
            "filepath": filepath,
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status != 200:
                    logger.warning(f"search_codebase_indexers_by_filepath HTTP {response.status}: {await response.text()}")
                    return None
                data = await response.json()
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.error(f"search_codebase_indexers_by_filepath request error: {e}")
            return None
    ...
    async def check_codebase_indexer_exists(self, codebase_indexer_id: str) -> Optional[CodebaseIndexerExistsResponse]:
        """
        Check if codebase indexer exists by ID.

        Args:
            codebase_indexer_id: Codebase indexer ID.

        Returns:
            CodebaseIndexerExistsResponse with exists flag, or None if request failed.
        """
        url = f"{self.base_url}/codebase_indexers/{codebase_indexer_id}/exists"
        try:
            await self._create_session()
            async with self.session.get(url) as response:
                if response.status != 200:
                    logger.warning(f"check_codebase_indexer_exists HTTP {response.status}: {await response.text()}")
                    return None
                data = await response.json()
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.error(f"check_codebase_indexer_exists request error: {e}")
            return None

        if data.get("status") != "success":
            return None

        exists_data = data.get("data", {})
        return CodebaseIndexerExistsResponse(
            status=data.get("status", ""),
            exists=exists_data.get("exists", False)
        )

    async def check_codebase_indexer_exists_by_dd_info(
        self,
        dd_namespace: str,
        dd_name: str
    ) -> Optional[CodebaseIndexerExistsResponse]:
        """
        Check if codebase indexer exists by DD namespace and name.
        ...
        Returns:
            CodebaseIndexerExistsResponse with exists flag, or None if request failed.
        """
        url = f"{self.base_url}/codebase_indexers/dd_info/{dd_namespace}/{dd_name}/exists"
        try:
            await self._create_session()
            async with self.session.get(url) as response:
                if response.status != 200:
                    logger.warning(f"check_codebase_indexer_exists_by_dd_info HTTP {response.status}: {await response.text()}")
                    return None
                data = await response.json()
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.error(f"check_codebase_indexer_exists_by_dd_info request error: {e}")
            return None
    ...
    async def get_codebase_indexer_count(self) -> Optional[CodebaseIndexerCountResponse]:
        """
        Get total count of codebase indexers.

        Returns:
            CodebaseIndexerCountResponse with total_count, or None if request failed.
        """
        url = f"{self.base_url}/codebase_indexers/status/count"
        try:
            await self._create_session()
            async with self.session.get(url) as response:
                if response.status != 200:
                    logger.warning(f"get_codebase_indexer_count HTTP {response.status}: {await response.text()}")
                    return None
                data = await response.json()
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.error(f"get_codebase_indexer_count request error: {e}")
            return None
        ...
        return CodebaseIndexerCountResponse(
            status=data.get("status", ""),

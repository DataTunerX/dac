...
from agent.compaction.prepare import (
    CompactionBoundary,
    CompactionResult,
...
from agent.compaction.settings import CompactionConfig
...
class CompactionGuard:
    ...
    One guard instance should be created per ``ReActRunner.run()`` invocation so
    ...
    async def _run_compact(
        ...
        *,
        ...
        if self.config.on_before_compact is not None:
            ...
            if isinstance(hook_result, dict):
                ...
                if isinstance(custom, dict) and custom.get("summary"):
                    from agent.compaction.prepare import rebuild_messages
                    from agent.compaction.tokens import estimate_messages_tokens

                    summary = str(custom["summary"])
                    details = custom.get("details") or {}
        ...
        logger.info(
            "compaction #%d DONE reason=%s tokens_before=%d tokens_after=%d reduction=%d%% summary_len=%d",
            self._compact_count,
            reason,
            result.tokens_before,
            result.estimated_tokens_after,

...
---------------------
- ``SG_REACT_COMPACTION_ENABLED`` : ``"true"`` (default) or ``"false"``.
- ``SG_REACT_CONTEXT_WINDOW`` : model context window size (default 200000).
"""
...
class CompactionConfig:
    """Public config passed into ``ReActRunner(..., compaction=...)``.
...
def context_window_from_env(default: int | None = None) -> int | None:
    """Read ``SG_REACT_CONTEXT_WINDOW`` if set, otherwise return ``default``.
    ...
    """
    raw = os.environ.get("SG_REACT_CONTEXT_WINDOW", "").strip()
    if not raw:
        return default
    try:
        value = int(raw)
...
def default_compaction_config() -> CompactionConfig:
    ...
    - ``SG_REACT_CONTEXT_WINDOW`` : model context window (default 200000).

    Returns:
        ``CompactionConfig`` with defaults suitable for the runner.
    """
    raw_enabled = os.environ.get("SG_REACT_COMPACTION_ENABLED", "true").strip().lower()

...
# 直连 data-services（默认 8000）
curl -X POST "http://192.168.3.7:22000/knowledge_graph/get_graph_by_source" \
  -H "Content-Type: application/json" \
  -d '{"source": "dac_dd202601281443", "node_limit": 1000, "rel_limit": 1000}' | jq .



curl -X POST "http://10.17.0.41:31337/knowledge_graph/get_graph_by_source" \
  -H "Content-Type: application/json" \
  -d '{"source": "default_dd01_gitee_testcode", "node_limit": 1000, "rel_limit": 1000}' | jq .

...
class PGVector(BaseVector):
    ...
    def get_all(self) -> list[Document]:
        with self._get_cursor() as cur:
            cur.execute(f"SELECT meta, text FROM {self.table_name}")
            docs = []
            for record in cur:
                metadata_str, text = record
                metadata = json.loads(metadata_str) if metadata_str else {}
    ...
    async def aget_all(self) -> list[Document]:
        conn = await self._get_async_connection()
        try:
            records = await conn.fetch(f"SELECT meta, text FROM {self.table_name}")
            docs = []
            for record in records:
                metadata_str, text = record
                metadata = json.loads(metadata_str) if metadata_str else {}
    ...
    def get_ids_by_metadata_field(self, key: str, value: str) -> list:
        with self._get_cursor() as cur:
            cur.execute(
                f"SELECT id::text FROM {self.table_name} WHERE meta->>%s = %s",
                (key, value)
            )
            return [row[0] for row in cur.fetchall()]

    async def aget_ids_by_metadata_field(self, key: str, value: str) -> list:
        conn = await self._get_async_connection()
        try:
            rows = await conn.fetch(
                f"SELECT id::text FROM {self.table_name} WHERE meta->>$1 = $2",
                key, value
            )

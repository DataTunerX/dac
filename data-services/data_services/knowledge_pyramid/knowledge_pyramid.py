...
class KnowledgePyramidService:
    ...
    def get_vector_instances(self, collection_names: list[str]) -> List[Vector]:
        """Get or create multiple Vector instances (with connection pool reuse)"""
        instances = []
        
        for collection_name in collection_names:
            instance = self.get_vector_instance(collection_name)
    ...
    async def find_metadata_values_in_collections(
        ...
        """Search metadata values in multiple collections concurrently"""
        
        # 获取所有 Vector 实例
        vectors = self.get_vector_instances(collection_names)
        
        # 并行执行所有查询
        try:
            # 使用 asyncio.gather 并行执行所有查询
            tasks = [
                vector.afind_metadata_values_in_collection(metadata_key=metadata_key)
            ...
            for i, result in enumerate(vector_results):
                ...
                if isinstance(result, Exception):
                    # 记录错误但继续处理其他结果
                    errors[collection_name] = str(result)
                    logger.warning(f"Error querying collection '{collection_name}': {str(result)}")
                else:
            ...
            if not successful_results and errors:
                raise Exception(f"All queries failed: {errors}")
            
            # 合并所有成功的结果
            all_results = {}
            for item in successful_results:
                all_results[item["collection"]] = item["result"]
    ...
    async def get_all_documents_with_knowledge_pyramid(self, collection_name: str) -> Dict[str, Any]:
        vector = self.get_vector_instance(collection_name)

        vector_results = []
        try:
            vector_results = await vector.aget_all()
    ...
    # search knowledge pyramid
    async def search_documents_with_knowledge_pyramid(self, query: str, collection_name: str, search_type: str, limit: int = 10, hybrid_threshold: Optional[float] = 0.01, vector_weight: Optional[float] = 0.7, fulltext_weight:Optional[float] = 0.3) -> Dict[str, Any]:
        vector = self.get_vector_instance(collection_name)
    ...
    async def delete_documents_by_ids(self, collection_name: str, documents: List[str]) -> Dict[str, Any]:
        ...
        logger.info(f"delete_documents_by_ids ,collection_name: {collection_name} ,documents, {documents_list}")

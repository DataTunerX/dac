"""SG-side capability broadcast (adapted for skill-agent).

The skill-agent can broadcast capability checks to discover peer agents
that can handle or contribute to a query when it cannot fully satisfy it.
"""
...
async def list_all_orchestrator_agent_cards(
    *,
    ...
    """List all registered agent cards from the registry.

    Uses ``biz_expert_agent_cards`` as default collection so skill-agent
    can discover all agents, not just orchestrators.
    """
    client = AgentRegistryClient()
    coll = collection_name or os.getenv("AgentRegistryCollection", "biz_expert_agent_cards")
    try:
        raw = await client.alist_all_agents(collection=coll)
        cards = _parse_agent_cards_from_response(raw)
        logger.info(
            "SkillAgent broadcast: list_all_agent_cards collection=%s count=%d",
            coll,
            len(cards),
        )
        return cards
    except Exception as e:
        logger.error("SkillAgent broadcast: list_all_agent_cards failed: %s", e)
        return []
...
async def purge_unreachable_agent_card(
    ...
    *,
    ...
    try:
        ...
        logger.warning(
            "SkillAgent broadcast: purged unreachable agent from registry | name=%s url=%s "
            "error=%s result=%s",
        ...
        logger.error(
            "SkillAgent broadcast: failed to purge unreachable agent | name=%s url=%s err=%s",
            getattr(agent_card, "name", ""),
            url,
            purge_exc,
        )
...
async def send_capability_check(
    ...
    *,
    ...
    try:
        ...
        logger.error(
            "SkillAgent broadcast: JSON parse error for agent %s (%s): %s",
            agent_card.name,
            agent_card.url,
            e,
        )
        ...
        logger.error(
            "SkillAgent broadcast: capability check failed for agent %s (%s): %s",
            agent_card.name,
            agent_card.url,
            e,
        )
...
async def broadcast_capability_check(
    ...
    *,
    ...
    if not all_agent_cards:
        logger.warning("SkillAgent broadcast: no agents in registry")
        return []

    logger.info(
        "SkillAgent broadcast: sending capability check to %d agents for query: %s...",
        len(all_agent_cards),
        (query or "")[:100],
    )
    tasks = [
    ...
    for entry in pool or []:
        ...
        if isinstance(agent_data, dict):
            try:
                ...
                logger.warning("SkillAgent broadcast: invalid agent snapshot for %s: %s", name, e)
    return out

...
class SemanticGroupClient:
    ...
    def get_leaf_groups_without_parent(self) -> Dict[str, Any]:
        """Get leaf groups that have SD members but no parent (candidates for hierarchical merging)."""
        endpoint = "/semantic_groups_leaf_orphans"
        return self._make_request("GET", endpoint)

    def get_orphan_groups_with_members(self) -> Dict[str, Any]:
        """Get all orphan groups (parent_id IS NULL) with at least one member (SD or child group)."""
        endpoint = "/semantic_groups_orphans_with_members"
        return self._make_request("GET", endpoint)

    def get_children_by_parent_id(self, parent_id: str) -> Dict[str, Any]:
        """Get direct child groups of a parent group."""
        endpoint = f"/semantic_groups/{parent_id}/children"
        return self._make_request("GET", endpoint)

    def update_parent_id(self, group_id: str, parent_id: Optional[str]) -> Dict[str, Any]:

...
def _read_batch_size(env_name: str, default: int) -> int:
    """Read an integer batch size from an environment variable with sane bounds."""
    try:
        value = int(os.getenv(env_name, str(default)))
    except (TypeError, ValueError):
        logger.warning(f"{env_name} 非法，使用默认值 {default}。当前值: {os.getenv(env_name)}")
        return default
    if value <= 0:
        logger.warning(f"{env_name} 不能小于等于 0，使用默认值 {default}。当前值: {value}")
        return default
    if value > 100:
        logger.warning(f"{env_name} 过大，限制为最大 100。当前值: {value}")
        return 100
    return value
...
class CodeAnalyzer:
    ...
    def ddd(self, content):
        ...
        ## ⚠️ 关键约束（最高优先级） ##
        以下参考样本数据仅为输出格式和风格示例。你必须**严格基于输入中实际存在的数据表和业务逻辑进行分析**，**绝对不能**将样本数据中的领域名称（如"用户管理"、"商品管理"、"订单管理"）、表名（如 `users`、`categories`、`products`、`orders`）引入到最终输出中，除非输入数据中确实包含了对应的表和业务领域。只分析输入中真实存在的内容。
        ...
        **核心职责：** 【该领域的业务职责】
        ...
        | 能力类型 | 能力名称 | 业务描述 | 输入 | 输出 | 关键业务规则 |
        | :--- | :--- | :--- | :--- | :--- | :--- |
        | **命令** | **操作A** | 业务流程描述 | 所需输入参数 | 预期输出 | 关键规则说明 |
        | **命令** | **操作B** | 业务流程描述 | 所需输入参数 | 预期输出 | 关键规则说明 |
        | **查询** | **查询A** | 数据查询描述 | 查询条件 | 查询结果 | 访问控制规则 |

        ### 🗣️ 领域语言与术语：

        | 类型 | 术语 | 领域语言描述 |
        | :--- | :--- | :--- |
        | **聚合根** | **实体A** | 代表系统中的核心业务实体 |
        | **值对象** | **值对象A** | 封装相关的属性组 |
        | **术语** | **业务术语A** | 该术语的业务含义说明 |
        ...
        | 模型类型 | 模型名称 | 所有属性和职责 |
        | :--- | :--- | :--- |
        | **聚合根** | **实体A** | 属性列表<br>职责：核心业务职责描述 |
        | **值对象** | **值对象A** | 属性列表<br>职责：封装业务规则 |

        ### ⚙️ 领域服务：

        | 服务名称 | 职责描述 | 涉及的业务规则 |
        | :--- | :--- | :--- |
        | **领域服务A** | 处理某业务流程 | 涉及的核心业务规则 |
        | **领域服务B** | 处理某业务流程 | 涉及的核心业务规则 |
    ...
    def ddd_old(self, content):
        ...
        【以下为输出格式和结构的参考示例。领域名称和具体表名均使用占位符，你必须基于输入中的真实数据来替换它们，不要直接引用下面的占位符名称。】

        该系统的核心业务范围是【基于输入推断的业务领域概述】。

        系统的最高层语义域是：【系统整体名称——基于输入总结】
        
        根据输入数据，系统可以划分为以下N个主要的**语义域**（即限界上下文，N取决于输入中实际涉及的业务范围）：

        ---

        ## 1. 【领域名称-1】（从输入数据中推断的领域名称）
        ...
        **核心职责：** 【该领域负责的业务】
        ...
        | :--- | :--- | :--- |
        | **术语** | **术语X** | 基于输入中表字段或注释推断出的业务术语含义。 |
        | **术语** | **术语Y** | 基于输入中表字段或注释推断出的业务术语含义。 |
        ...
        | 模型类型 | 模型名称 | 所有属性和职责 |
        | :--- | :--- | :--- |
        | **聚合根** | **实体A** | 根据输入中的实际表推断的实体职责。 |
        | **聚合根** | **实体B** | 根据输入中的实际表推断的实体职责。 |

        ### 模型关系：

        * **`实体A`** 与 **`实体B`** 的关联关系（仅当输入中有相应的外键或关系描述）。

        ---

        ## 2. 【领域名称-2】

        *(同上结构，仅当输入中真实存在对应的表或业务模块时才添加)*

        ---

        ## 🌐 跨上下文关系总结 (Context Mapping)

        以下是不同限界上下文之间的关键协作和依赖关系（仅当存在多个领域时）：

        | 源上下文 | 目标上下文 | 关系说明 |
        | :--- | :--- | :--- |
        | **领域A** | **领域B** | 基于输入数据的实际关联说明。 |

        ### 补充说明：

        1. (仅展开输入中真实存在的上下文关系和业务规则)
    ...
    def agent_card(self, content):
        ...
        ### ⚠️ 最高优先级约束（必须先读完再输出） ###
        **严格基于输入内容进行分析，只描述输入中真实存在的子领域、业务模块和数据实体。绝对不要编造输入中不存在的业务领域，即使输入中有不严谨或模糊的表述，也不可据此自行推演出不存在的内容。只基于输入中实际存在的内容做分析。**
        ...
        **第二层：子领域逐项展开（本层是描述主体，约占全文 80%）**
        ...
        每个子领域都要实实在在展开到"把输入的每一张表/字段说明都映射进去"的程度；输入有多少个有业务含义的子领域，本层就要有多少个小节，**一个都不能少**。
...
class SQLAnalyzer:
    ...
    def tables_domains(self, tables_overview) -> dict:
        ...
        ## ⚠️ 关键约束（最高优先级） ##
        以下JSON格式示例中出现的语义域名称（如"用户管理"、"商品管理"、"订单管理"）仅作为格式参考。你必须**严格基于输入中实际存在的表名来划分语义域**，不能将示例中的领域名直接引入输出，除非输入数据确实包含对应的表。
        ...
        **⚠️ 以下示例中的语义域名称（如"领域A"、"领域B"）和 business_meaning 内容仅为格式参考。你必须基于输入中实际存在的表名来命名语义域和描述业务含义，不能直接复制示例中的领域名称或描述。**

        [
            {{
                "semantic_domain": "领域名称A（基于输入表推断）", "tables": ["table_x", "table_y"], "business_meaning":"基于输入中这两张表的实际字段和注释，描述它们共同负责的业务能力"
            }},
            {{
                "semantic_domain": "领域名称B（基于输入表推断）", "tables": ["table_m", "table_n"], "business_meaning":"基于输入中这两张表的实际字段和注释，描述它们共同负责的业务能力"
            }}
        ]
    ...
    def tables_ddd(self, tables_overview, tables_relationship) -> dict:
        ...
        ## ⚠️ 关键约束（最高优先级） ##
        以下参考样本数据**仅作为输出结构、格式和风格的示例**。你必须**只基于下面 HumanMessage 中提供的真实数据库表和关系进行分析**。绝对不能在输出中编造任何未出现在输入数据中的表名（如 users, products, categories）、字段名或领域名称。每个领域的描述只能来源于输入中真实存在的表。

        ## 以下是输出的格式参考样本（不要引用其中的具体表名或领域名）##

        该系统的核心业务范围是管理【业务领域描述——基于输入的真实表推断】。

        系统的最高层语义域是：【系统名称——基于输入的真实内容总结】
        
        根据表的功能和描述，系统可以划分为以下N个主要的**语义域**（即限界上下文, N由输入中实际表的数量决定）：

        ---

        ## 1. 【领域名称-1】（基于输入表推断出来的领域）
        ...
        **核心职责：** 【描述该领域实际负责的业务】
        ...
        | :--- | :--- | :--- | :--- |
        | **聚合根** | **实体名称A** | `table_x` | 从输入表中的实际字段推断的实体职责。 |
        | **聚合根** | **实体名称B** | `table_y` | 从输入表中的实际字段推断的实体职责。 |
        ...
        *(同上结构，仅当输入中真实存在对应的表时才添加此领域)*
        ...
        | :--- | :--- | :--- | :--- |
        | **领域A** | **领域B** | 基于输入关系描述的关联。 | `table_x` (外键字段) |
    ...
    def batch_process_schemas(self, schemas: List[Dict[str, Any]], batch_size: Optional[int] = None):
        """
        return:
        ...
        if batch_size is None:
            batch_size = _read_batch_size("TABLES_OVERVIEW_BATCH_SIZE", "20")
    ...
    def tables_group(self, content, relationships, db_and_code_ddd_summary:str = "", batch_size:int = 20) -> dict:
        ...
        for attempt in range(1, max_retries + 1):
            if attempt == 1:
                ...
                请严格按照以下要求修正：
                1. 每个分组项的 key 必须直接是模块名称（如 "业务模块A"），value 是表名列表
                2. 不允许出现 "业务模块": "业务模块A" 这种将模块名作为 value 的格式
                3. 格式示例：{{"业务模块A": ["table_x", "table_y"], "count": 2}}
    ...
    def _get_sql_prompt(self, content: str, chunk_notice: str = "") -> str:
        ...
        return f"""
            ...
            **⚠️ 以下 sample data 仅为输出格式参考。你必须基于输入中实际存在的表名和字段进行分析，不能将示例中的表名（如 table_a、table_b）或模块名直接复制到输出中，除非输入数据确实包含它们。**

            {{  
                "business_capabilities_summary": "【基于输入中实际表名和字段总结的业务能力概述，不引用示例中的内容】",
                "extracted_table_informations": [
                    {{
                        "table_name": "table_a（用输入中真实的表名替换）",
                        "fileds": ["field_x（字段x的中文业务名称）", "field_y（字段y的中文业务名称）", ...]
                    }},
                    {{
                        "table_name": "table_b（用输入中真实的表名替换）",
                        "fileds": ["field_m（字段m的中文业务名称）", "field_n（字段n的中文业务名称）", ...]
                    }},
                    ...
                ],
                "module_group_with_tables": [
                    {{
                        "模块名称A（基于输入推断）": ["table_a（表a中文名）", "table_b（表b中文名）", ...]
                    }},
                    {{
                        "模块名称B（基于输入推断）": ["table_c（表c中文名）", ...]
                    }},
                    ...
                ],
                "tables_relationships": "
                    ...
                    |------------|-------------------------|---------------------------|--------------------|
                    | **关系类型** | table_a → table_b | fk_field → pk_field | constraint_name |
                "
            }}
...
- 不要添加原始数据中不存在的关系"""
    ...
    def generate_table_relationship(self, content: str) -> str:
        ...
        Returns:
            **Specific Relationship Details**
            ...
            | 关系类型      | 技术关联                                | 自然语言描述                                                                 |
            |---------------|----------------------------------------|------------------------------------------------------------------------------|
            | 自引用        | table_e.parent_col → table_e.pk_col | 示例说明：表内部通过父子列形成层级关系。       |
            | 一对多        | table_a.fk_col → table_b.pk_col | 示例说明：A表通过外键列关联到B表。          |
            | 一对多        | table_c.fk_col → table_d.pk_col | 示例说明：C表通过外键列关联到D表。          |
            | 自引用        | table_e.parent_col → table_e.pk_col | 示例说明：表内部通过父子列形成层级关系。       |
        """
        try:    
            ...
            | 关系类型 | 技术关联 | 自然语言描述 |
            |---------|---------|------------|
            | 自引用   | table_e.parent_col → table_e.pk_col | 表内部通过父子列关联，形成层级关系。 |
            | 一对多   | table_a.fk_col → table_b.pk_col | 一个A记录可以包含多个B记录，每个B通过外键关联到A。 |
            | 一对多   | table_c.fk_col → table_d.pk_col | 一个C记录可以被多个D引用，每个D通过外键确定对应的C。 |
    ...
    def generate_tables_summary(self, content: str, db_and_code_ddd_summary: str, max_retries: int = 3):
        ...
        prompt_ddd = f"""
            ...
            **⚠️ 以下示例仅展示JSON结构和字段格式。所有领域名、表名、术语和描述都是占位符，你必须根据输入中真实存在的表来替换，绝对禁止将示例中的任何具体名称（如 用户管理、User、users、订单管理 等）复制到输出中。**

            [
              {{
                "domain_overview": {{
                  "domain_name": "领域名称A（从系统DDD文档和当前批次表中推断）",
                  "tables": ["table_x", "table_y"],
                  "business_scope": "基于输入中这些表的实际业务语义来描述业务范围"
                }},
                "domain_details": {{
                  "core_responsibility": "基于输入中表的实际内容和DDD文档来描述核心职责",
                  "ubiquitous_language": [
                    {{
                      "type": "术语",
                      "term": "术语X",
                      "description": "基于输入中表的实际字段和注释来描述"
                    }},
                    {{
                      "type": "术语",
                      "term": "术语Y",
                  ...
                  "domain_model": [
                    {{
                      "model_type": "聚合根",
                      "model_name": "实体A",
                      "key_attributes_and_responsibilities": "基于输入中表的实际字段和职责来描述"
                    }}
                  ],
                  "model_relationships": "基于输入中实际的外键关系来描述模型之间的关联。"
                }},
                "relationship_analysis": {{
                  "internal_model_relationships": "基于输入中真实的外键和关系描述。",
                  "external_context_dependencies": "基于输入中真实的关系描述来说明与其他上下文的依赖。"
                }},
                "business_capabilities": {{
                  "summary": "基于输入中表的实际业务能力来总结。",
                  "data_flow": "基于输入中表的字段和关系来描述核心数据流转。"
                }}
              }},
              //更多的domains........
            ]
    ...
    def agent_card(self, content):
        ...
        ### ⚠️ 最高优先级约束（必须先读完再输出） ###
        **严格基于输入内容进行分析。核心判断原则：只有输入中明确给出了完整表结构定义（如 CREATE TABLE 等 DDL）的数据表，才属于本 Agent 负责的子领域。仅仅在字段注释、外键描述或关系说明中被"提及"的外部表（例如 `user_id INT COMMENT '关联 users 表'`），不代表该外部表对应的领域属于本系统——它们只是外部依赖引用，绝对不能据此在 description 或 skills 中展开一个独立的子领域。如果输入中的某个领域名称（如"用户管理"）只在跨上下文关系的描述中出现、但没有对应的 DDL 表定义，则视为外部领域引用，不得展开描述。**
...
class FileAnalyzer:
    ...
    def agent_card(self, content):
        ...
        ### ⚠️ 最高优先级约束（必须先读完再输出） ###
        **严格基于输入内容进行分析，只描述输入中真实存在的子领域、业务模块和数据实体。绝对不要编造输入中不存在的业务领域，即使输入中有不严谨或模糊的表述，也不可据此自行推演出不存在的内容。仅在被提及的文档中明确存在的领域和实体才能被描述。**

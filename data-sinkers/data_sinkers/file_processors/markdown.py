...
class MarkdownProcessor:
    ...
    def __init__(self):
        logger.info(f"MarkdownProcessor init")
    ...
    def load_with_unstructured(self, file_path: str, **kwargs) -> List[Document]:
        try:
            ...
            logger.info(f"UnstructuredMarkdownLoader loaded {len(documents)} pages from {os.path.basename(file_path)}")
            return documents
        except Exception as e:
            logger.error(f"UnstructuredMarkdownLoader failed: {e}")
            return []
        ...
        **kwargs
        ...
        for file_path in file_paths:
            try:
                ...
                split_docs = self.process_markdown(file_path, loader_type, **kwargs)
